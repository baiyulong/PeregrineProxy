...
pub async fn handle_udp_associate(tcp_stream: &mut TcpStream, _request: &SocksRequest) -> ProxyResult<()> {
...
async fn handle_udp_packet(
    ...
    tracing::debug!("UDP packet from {} to {:?}, payload len: {}", client_addr, target_addr, payload.len());
    ...
    let target_socket = match client_map.get(&client_addr) {
        ...
        None => {
            ...
            let target_socket = Arc::new(UdpSocket::bind("0.0.0.0:0").await.map_err(ProxyError::Io)?);
            ...
            tokio::spawn(async move {
                listen_for_responses(relay_socket_clone, target_socket_clone, client_addr, target_addr_clone, client_map_clone).await;
            });
    ...
    let _ = target_socket.send_to(payload, target_socket_addr).await.map_err(ProxyError::Io)?;
...
async fn resolve_target_addr(target: &TargetAddr) -> ProxyResult<SocketAddr> {
    match target {
        ...
        TargetAddr::Ipv6(ip, port) => {
            ...
            Ok(SocketAddr::V6(std::net::SocketAddrV6::new(addr, *port, 0, 0)))
        }
        TargetAddr::Domain(domain, port) => {
