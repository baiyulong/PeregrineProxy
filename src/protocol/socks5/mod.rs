...
pub async fn handle_socks5(stream: TcpStream, addr: SocketAddr, users: Option<Vec<UserCredential>>) {
    if let Err(e) = handle_socks5_inner(stream, addr, users).await {
        tracing::error!("SOCKS5 error from {}: {}", addr, e);
    }
}

async fn handle_socks5_inner(mut stream: TcpStream, _addr: SocketAddr, users: Option<Vec<UserCredential>>) -> crate::error::ProxyResult<()> {
    ...
    if method == 0x02 {
        if let Some(ref user_list) = users {
            ...
            return Err(crate::error::ProxyError::Socks5("Authentication required but no users configured".into()));
        }
    }
