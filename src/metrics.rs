...
impl ProxyMetrics {
    ...
    pub async fn start_server(metrics: Arc<Self>, listen_addr: String) -> tokio::task::JoinHandle<()> {
                    ...
                    Ok((mut socket, _)) => {
                        ...
                        tokio::spawn(async move {
                            ...
                            match socket.read(&mut buffer).await {
                                ...
                                Err(e) => {
                                    tracing::debug!("Failed to read from metrics connection: {}", e);
                                }
                            }
                        });
                    }
...
mod tests {
    ...
    fn test_family_metrics_with_different_labels() {
        ...
        metrics.requests_total.get_or_create(&http_forward).inc_by(5);
        metrics.requests_total.get_or_create(&socks5_connect).inc_by(3);
        
        assert_eq!(metrics.requests_total.get_or_create(&http_forward).get(), 5);
        assert_eq!(metrics.requests_total.get_or_create(&socks5_connect).get(), 3);
    }
