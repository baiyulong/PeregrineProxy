...
impl UpstreamConnector for ChainConnector {
    async fn connect(&self, target: &ConnectTarget) -> ProxyResult<BoxedStream> {
            ...
            return match &self.chain[0] {
                    ...
                    match target {
                        ConnectTarget::Address(host, port) => {
                            let stream = TcpStream::connect(format!("{}:{}", host, port)).await
                                .map_err(|e| ProxyError::Upstream(format!("Direct connection failed: {}", e)))?;
                ...
                config => {
                    ...
                    let stream = TcpStream::connect(&proxy_addr).await
                        .map_err(|e| ProxyError::Upstream(format!("Failed to connect to proxy {}: {}", proxy_addr, e)))?;
        ...
        let stream = TcpStream::connect(&first_proxy_addr).await
            .map_err(|e| ProxyError::Upstream(format!("Failed to connect to first proxy {}: {}", first_proxy_addr, e)))?;
        ...
        for i in 0..self.chain.len() {
            ...
            boxed_stream = perform_proxy_handshake(boxed_stream, &self.chain[i], &next_target).await?;
        }
...
fn extract_proxy_address(config: &UpstreamConfig) -> ProxyResult<String> {
    match config {
        UpstreamConfig::Direct => Err(ProxyError::Upstream("Direct config has no proxy address".into())),
        UpstreamConfig::Http { addr, .. } => Ok(addr.clone()),
        UpstreamConfig::Socks5 { addr, .. } => Ok(addr.clone()),
        UpstreamConfig::Chain { .. } => Err(ProxyError::Upstream("Nested chains not supported".into())),
    }
}
...
fn parse_addr_to_target(addr: &str) -> ProxyResult<ConnectTarget> {
    if let Some(colon_pos) = addr.rfind(':') {
        ...
        Err(ProxyError::Upstream(format!("Address missing port: {}", addr)))
    }
}
...
async fn perform_socks5_handshake(
    ...
    if resp[0] != 0x05 {
        return Err(ProxyError::Socks5("Invalid SOCKS5 version from upstream".into()));
    }
    ...
    match resp[1] {
        ...
        0xFF => return Err(ProxyError::Socks5("Upstream SOCKS5 proxy rejected all auth methods".into())),
        other => return Err(ProxyError::Socks5(format!("Unsupported auth method from upstream: {}", other))),
    }
    ...
    match target {
        ConnectTarget::Address(host, port) => {
            ...
            stream.read_exact(&mut reply_header).await.map_err(ProxyError::Io)?;
            ...
            if reply_header[1] != 0x00 {
                ...
                return Err(ProxyError::Upstream(format!("SOCKS5 upstream CONNECT failed: {}", err_msg)));
            }
...
mod tests {
    ...
    async fn test_chain_connector_empty_chain_fails() {
        ...
        let result = connector.connect(&ConnectTarget::Address(
    ...
    async fn test_chain_connector_single_direct_hop() {
        ...
        stream.write_all(b"Hello via chain (direct)!").await.unwrap();
    ...
    async fn test_chain_connector_three_hop() {
        ...
        use tokio::io::{AsyncWriteExt as _, AsyncReadExt as _};
        stream.write_all(b"Hello via three-hop chain!").await.unwrap();
