...
fn test_check_proxy_auth_correct_credentials() {
    let users = vec![
        UserCredential { username: "testuser".into(), password: "testpass".into() },
        UserCredential { username: "admin".into(), password: "secret".into() },
    ];
...
async fn test_make_407_response() {
    ...
    assert!(auth_header.is_some(), "Should have Proxy-Authenticate header");
