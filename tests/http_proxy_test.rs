...
async fn test_http_proxy_forward() {
    ...
    assert!(
        response_str.contains("Hello, Proxy!"),
        "Expected body, got: {}",
        response_str
    );
}
...
async fn test_http_connect_tunnel() {
    ...
    assert_eq!(
        &echo[..n],
        test_data,
        "Data should be echoed through tunnel"
    );
}
    ...
    .await
    {
        Ok(Ok(mut tunnel)) => {
            ...
            assert_eq!(
                &echo[..n],
                test_data,
                "Data should echo through upstream tunnel"
            );
        }
    ...
    .await
    {
        Ok(Ok(response)) => {
            assert!(
                ...
                "Expected 407 Proxy Authentication Required, got: {}",
                response
            );
            assert!(
