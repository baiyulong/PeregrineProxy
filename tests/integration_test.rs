...
async fn test_integration_acl_deny_blocks_connection() {
    ...
    .await
    {
        Ok(Ok(response)) => {
            assert!(
                response.contains("403") || response.contains("Forbidden"),
    ...
    .await
    {
        Ok(Ok(response)) => {
            assert!(
                ...
                "Expected 407 auth challenge, got: {}",
                response
            );
            assert!(
    ...
    .await
    {
        Ok(Ok(mut tunnel)) => {
            ...
            assert_eq!(
                &echo[..n],
                test_data,
                "SOCKS5 should work on different port"
            );
        }
    ...
    .await
    {
        Ok(Ok(response)) => {
            ...
            assert!(
                ...
                "Expected error response for unreachable target, got: {}",
                response
            );
        }
        Ok(Err(_)) => {
